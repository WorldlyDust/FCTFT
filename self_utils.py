...
class TrainDataset(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=True, stride=8):
        ...
        for i in tqdm(range(len(hyper_list))):
            ...
            hyper = np.transpose(hyper, [0, 2, 1])# [31,482,512]
...
class ImprovedLoss(nn.Module):
    def __init__(self, alpha=0.8):
        """
        初始化改进的损失函数
        :param alpha: MRAE损失的权重，范围在0到1之间，用于平衡MRAE和MSE的贡献
        """
    ...
    def forward(self, outputs, label):
        """
        前向传播计算损失
        :param outputs: 模型的输出，即重建的高光谱图像
        :param label: 真实的高光谱图像标签
        :return: 改进后的损失值
        """
        # 确保输出和标签的形状一致
        assert outputs.shape == label.shape
        ...
        # 计算MSE损失
        mse = nn.MSELoss()(outputs, label)

        # 结合MRAE和MSE损失
        loss = self.alpha * mrae + (1 - self.alpha) * mse
        return loss
...
class Loss_Wavelet(nn.Module):
    ...
    def forward(self, im_true, im_fake):
        ...
        im_true_w =self.dwt(self.dwt(self.dwt(im_true)))
        im_fake_w =self.dwt(self.dwt(self.dwt(im_fake)))
